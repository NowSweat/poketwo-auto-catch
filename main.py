...
with open("data/config.txt","r") as file:
    ...
    spam_channel = info["spam_channel"]
...
def spam():
    while True:
        bot.sendMessage(spam_channel, version)
        time.sleep(1)
import sys
...
update_title("Nuevo Título de la Terminal")
@bot.gateway.command
def on_ready(resp):
            ...
            if m["author"]["id"] == poketwo_id:# If poketwo sends a message
                ...
                if m["embeds"]:# If message is an embedded message
                    ...
                    if "A wild pokémon has appeared!" in embed_title:# If wild pokemon appears
                        ...
                        global num_fled
                        num_fled += 1
                        update_title(f"Pokemon Caught: {num_pokemon} || Shinies: {num_shinies} || Legendaries: {num_legendaries} || Mythics: {num_mythics} || Fled: {num_fled}")
                    ...
                    if "The pokémon is " in content:# If message is a hint
                        ...
                        if len(solution) == 0:
                            ...
                            for i in range(0,len(solution)):
                                time.sleep(2)
                                bot.sendMessage(channel_id,"<@716390085896962058> c " + solution[i])
